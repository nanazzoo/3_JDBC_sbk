...
//			없으면 "조회 결과가 없습니다" 출력
			if(memberList.isEmpty()) { //비어있는 경우
				...
				for(Member mem : memberList) {
					if(mem.getMemberGender().equals("M")) gender = "남";
					else gender = "여";
					System.out.printf("| %10s | %9s | %11s |\n", 
							mem.getMemberId(), mem.getMemberName(), gender);
				}
			}
//			printAll(memberList);
		} catch (Exception e) {
...
//			서비스 호출 후 결과 반환받기
			int result = service.updatePw(loginMember.getMemberNo(), currentPw, newPw1);
//											   로그인한 회원 번호, 현재비밀번호, 새 비밀번호
